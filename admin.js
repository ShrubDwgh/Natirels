...
export function toggleServer(checked) {
  ...
  setDoc(doc(db, 'afk_config', 'server'), { online: state.serverOnline });
  applyServerStatus();
}
...
export async function saveSlots() {
  try { await setDoc(doc(db, 'afk_config', 'slots'), state.slotData); } catch(e) { console.error(e); }
...
export async function saveSettings() {
  ...
  try { await setDoc(doc(db, 'afk_config', 'settings'), state.settings); } catch(e) { console.error(e); }
  updateLinks();
